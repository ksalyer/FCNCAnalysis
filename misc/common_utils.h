...
float triggerScaleFactor(TFile* effFile, int year, int pdgId1, int pdgId2, float pt1, float pt2, float eta1, float eta2, float ht, int syst=0) {//ana_t ana, int syst=0) { 
    ...
    return yrun2::TriggerWeight(effFile, pdgId1, pt1, eta1, pdgId2, pt2, eta2, ht, year, true, syst);
}

float fastsim_triggerScaleFactor(TFile* effFile, int year, int pdgId1, int pdgId2, float pt1, float pt2, float eta1, float eta2, float ht, int syst=0) {
    ...
    return yrun2::TriggerWeight(effFile, pdgId1, pt1, eta1, pdgId2, pt2, eta2, ht, year, false, syst);
    // return 1.0;
}
...
float fakeRate(int year, int id, float pt, float eta, bool isData) { 
    if (year == 2016) return y2016::fakeRate(id, pt, eta, isData);
    else if (year == 2017) return y2017::fakeRate(id, pt, eta, isData);
...
float getIterativeBSF(int year, Jets &jets, Jets &bjets, BTagCalibrationReader &deepjet_reader, string variationName){
    if (year == 2016) return y2016::getIterativeBSF(year, jets, bjets, deepjet_reader, variationName);
    else if (year == 2017) return y2017::getIterativeBSF(year, jets, bjets, deepjet_reader, variationName);
    else if (year == 2018) return y2018::getIterativeBSF(year, jets, bjets, deepjet_reader, variationName);
    else {throw std::runtime_error("ControlTree::INIT: Error - invalid year");}
}
